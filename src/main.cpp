...
namespace typescriptwinmd
{
    ...
    {
        { "input", 1, option::no_max, "<spec>", "Windows metadata to include in projection" },
        { "reference", 0, option::no_max, "<spec>", "Windows metadata to reference from projection" },
        { "outputPath", 1, 1, "<path>", "Folder for generated definition files" },
...
Where <spec> is one or more of:
  ...
  10.0.12345.0[+]     Specific version of Windows SDK [with extensions]
        ...
        w.write(format, TYPESCRIPTWINMD_VERSION_STRING, bind_each(printOption, options));
    }
    ...
    static void process_args(int const argc, char** argv)
    {
        ...
        auto output_folder = canonical(args.value("outputPath"));
        output_folder += '/';
    ...
    static int run(int const argc, char** argv)
    {
        ...
        try
        {
            ...
            if (settings.verbose)
            {
                ...
                w.write(" ver:   %\n", TYPESCRIPTWINMD_VERSION_STRING);
            ...
            for (auto&&[ns, members] : c.namespaces())
            {
                ...
                {
                    if (!settings.projection_filter.includes(members) || !has_projected_types(members))
                    {
                        return;
                    }
...
{
    return typescriptwinmd::run(argc, argv);
}
